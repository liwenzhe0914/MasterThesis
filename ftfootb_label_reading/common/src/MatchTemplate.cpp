...
void MatchTemplate::load_templates(const std::string& templates_storage_path, std::map<std::string, cv::Mat>& template_data_structure)
{
...
//	std::string imgpath = arg.substr(0, arg.find_last_of("/") + 1);
	...
	for (unsigned int i = 0; i < templateImageNames.size(); i++)
	{
		...
		template_data_structure[rawname] = cv::imread(templateImageNames[i], CV_LOAD_IMAGE_COLOR);			// todo: check whether color image is needed
	}
...
void MatchTemplate::match_token_templates(const cv::Mat& image, const TokenTemplates& token_templates, std::multimap<double, std::string>& matching_scores)
{
	...
	for (TokenTemplates::const_iterator it=token_templates.begin(); it!=token_templates.end(); ++it)
	{
		...
		cv::Mat match_matrix(image.rows - template_resized.rows + 1, image.cols - template_resized.cols + 1, CV_32FC1);
		cv::matchTemplate(image, template_resized, match_matrix, match_method);
