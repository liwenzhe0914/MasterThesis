...
//	text_tags_cascade.detectMultiScale( image, texts, reject_levels, level_weights,1.04,6,0, cv::Size(35, 9),cv::Size(), true );
	text_tags_cascade.detectMultiScale( image, texts, 1.05 ,1,0, cv::Size(35, 9),cv::Size());
...
//		std::cout<<rectangles_list.size()<<std::endl;
		rectangle( img_color, cv::Point(texts[i].x,texts[i].y), cv::Point(texts[i].x + texts[i].width,texts[i].y+texts[i].height), cv::Scalar( 255, 0, 255 ) , 2 , 8 , 0 );
//		//rectangle(img_color, texts[i], cv::Scalar(255,255,255), 3 , 8 , 0);
